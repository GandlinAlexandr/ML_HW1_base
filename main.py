...
X_train = transform_data(df_train.drop(['selling_price', 'torque', 'name'], axis=1))
...
encoded_train = encoder.fit_transform(X_train.select_dtypes('object')).toarray()
...
X_train_scaler = scaler.transform(X_train_encoded)
...
def predict_item(item: Item) -> float:
    ...
    input_data = pd.DataFrame([item.dict()]).drop(['selling_price', 'torque', 'name'], axis=1)
    # Обработка
    ...
    # OHE
    encoded_data = encoder.transform(data.select_dtypes('object')).toarray()
    ...
    data_cat_encoded = pd.concat(
      ...
      axis=1)
    # Скалирование
    data_cat_encoded_scal = scaler.transform(data_cat_encoded)
    # Предсказания модели
    predicted_price = model.predict(data_cat_encoded_scal)
...
def predict_items(items: Items) -> List[float]:
    ...
    input_data = input_data.drop(['selling_price', 'torque', 'name'], axis=1)
    # Обработка
    data = transform_data(input_data)
    data = fill_NA(data, train_median_mileage, train_median_engine, train_median_max_power, train_median_seats)
    # OHE
    ...
    data_cat_encoded_scal = scaler.transform(data_cat_encoded)
    # Предсказания модели
    predicted_prices = model.predict(data_cat_encoded_scal)
