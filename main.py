...
def predict_item(item: Item) -> float:
    ...
    # Обработка
    input_data = fill_NA(input_data, train_median_mileage, train_median_engine, train_median_max_power, train_median_seats)
...
def predict_items(items: Items) -> List[float]:
    ...
    # Обработка
    data = fill_NA(input_data, train_median_mileage, train_median_engine, train_median_max_power, train_median_seats)
...
def upload(file: UploadFile = File(...)):
    input_data = pd.read_csv(file.file, index_col=0)
    # Конвертация в датафрейм и удаление ненужных столбцов
    data = input_data.drop(['selling_price', 'torque'], axis=1)
    # Обработка
    ...
    # Предсказания модели
    predicted_prices = model.predict(data_cat_encoded_scal.values)
    # Возвращаемся от логарифмированного таргета к изначальным масштабам
    predicted_prices = [float(round(price, 0)) for price in np.e ** predicted_prices]
    # Добавление предсказанных значений в изначальный датасет
    output = input_data.copy()
    output['predict'] = predicted_prices
    stream = io.StringIO()
